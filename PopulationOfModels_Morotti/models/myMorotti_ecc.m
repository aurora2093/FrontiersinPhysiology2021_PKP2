function ydot = myMorotti_ecc(t,y,p, cond,PKP2flag,experimflag,CabFac)
...
if PKP2, Vjunc = Vjunc*0.60; end % following discussion Thomas
...
IbarNCX = cond(7);
% if experimFlag, IbarNCX = IbarNCX*2; end
if CKIIflag == 1,
    IbarNCX = 1.5*IbarNCX;
end
KmCai = 3.59e-3;    % [mM]
...
if PKP2, Bmax_Csqn = Bmax_Csqn*45.3/100; end
koff_csqn = 65;         % [1/ms] 
...
if CKIIflag == 1, % MOUSE (chronic CaMKII-OE effect)
    I_ki = 1/2*Gk1*kiss*(y(39)-ek)*Kcoeff;
else
    I_ki = Gk1*kiss*(y(39)-ek)*Kcoeff;
end
...
if PKP2, ICa_scale = ICa_scale*0.5; end 
...
if PKP2, r2 = r2*0.75; end %%% following discussion Thomas
...
% if PKP2, I_Ca_junc = I_Ca_junc*50/100; I_Ca_sl = I_Ca_sl*50/100; end
...
IbarNCXjunc = IbarNCX; IbarNCXsl = IbarNCX; 
% if experimFlag, IbarNCXjunc = IbarNCX * 2; end 
I_ncx_junc = Fjunc_ncx*IbarNCXjunc*Q10NCX^Qpow*Ka_junc*(s1_junc-s2_junc)/s3_junc;
I_ncx_sl = Fsl_ncx*IbarNCXsl*Q10NCX^Qpow*Ka_sl*(s1_sl-s2_sl)/s3_sl;
...
RI = 1-y(14)-y(15)-y(16);
fRyR = cond(10);
% if experimFlag, koSRCa= koSRCa*20/100; end %
...
J_SRCarel = fRyR*ks*y(15)*(y(31)-y(36));          % [mmol/L SR/ ms]
if PKP2, J_SRCarel = J_SRCarel*60/100; end 
% if experimFlag, J_SRCarel= J_SRCarel*20/100; end % 30% reduction
...
J_SRleak = fRyR*kleak*(y(31)-y(36)); % [mmol/L cyt/ms]
if PKP2, J_SRleak = J_SRleak*60/100; end %%%% discussion with Thomas 
% if experimFlag, J_SRleak = J_SRleak*20/100; end % 30% reduction

global Jleak 
Jleak(tStep,1) = J_SRCarel*Vsr/Vmyo + J_SRleak; % Total leak [mmol/L cyt/ms]
Jleak(tStep,2) = J_SRleak;                      % Passive leak only [mmol/L cyt/ms]  
global Jrel
Jrel(tStep) = J_SRCarel; 
...
fSerca = cond(9);
J_serca = fSerca*Q10SRCaP^Qpow*Vmax_SRCaP*((y(38)/Kmf)^hillSRCaP-(y(31)/Kmr)^hillSRCaP)...
...
% 		V_hold = -90;
        V_hold = -50; % CHANGED
        V_test = 20; %20 % Use 0.05 for 0 mV to avoid numerical problems
        V_step = -50;
        INa_inactT = 30; % ms
