...
PKP2flag =0; experimflag = 0; BARSflag = 0; 
...
elseif model == 3
    mod = @myMorotti;
    meanParams = conductancesMorotti();
    savePath = 'myPoM/';
else
...
%               [result(i).time{j} , X]=ode15s(mod,[0 CL(j)],X0,options,p,params(i,:));
              [result(i).time{j} , X]=ode15s(mod,[0 CL(j)],X0,options,p,params(i,:),PKP2flag,experimflag);
            X0=X(size(X,1),:);
        end
...
%         result(i).V{j} = X(:,1);
    ...
    time = result(i).time;
    yfin = result(i).yfin;
    parsave([savePath 'PoM' num2str(i) '.mat'],{V , time , params(i,:) , APD, CL, yfin});
    parfor_progress;
end
parfor_progress(0);
