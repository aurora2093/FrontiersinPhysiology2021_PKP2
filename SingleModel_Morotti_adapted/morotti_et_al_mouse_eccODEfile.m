function ydot = morotti_et_al_mouse_eccODEfile(t,y,p,clamp)
...
if experimFlag, GCaB = GCaB * 2.5; end
...
% if experimFlag, J_SRCarel= J_SRCarel*30/100; end % 30% reduction
...
% if experimFlag, J_SRleak = J_SRleak*30/100; end % 30% reduction
...
if clamp, ydot(31)=0; end %steadystate
...
if clamp, ydot(36)= 0 ; end %% steadystate
...
% 		V_hold = -90;
		...
		if mod(t,cycleLength) <= INa_inactT
		    V_clamp = V_step;
        elseif mod(t,cycleLength)>INa_inactT && mod(t,cycleLength)<= 600 + INa_inactT %200
		    V_clamp = V_test;
        else
            V_clamp = V_hold;
		end
